...
def split_edges(adj, ratio_val, ratio_test, rng):
...
def mask_test_edges(adj, ratio_val, ratio_test, balanced_metrics, rng):
    ...
    train_edges, val_edges, test_edges = split_edges(adj, ratio_val, ratio_test, rng)
    edges_all = sparse_to_tuple(adj)[0]

    if not balanced_metrics:
        neg_adj = sp.csr_matrix(np.ones(adj.shape) - adj)
        train_edges_false, val_edges_false, test_edges_false = split_edges(neg_adj, ratio_val, ratio_test, rng)
...
def gen_train_val_test_sets(adj, crossval, balanced_metrics, ratio_val, ratio_test, rng=np.random.RandomState()):
    ...
    if crossval and not balanced_metrics:
        for i in range(5):
            train_e, train_e_false, val_e, val_e_false, test_edges, test_edges_false = mask_test_edges(adj, ratio_val,
                                                                                                       ratio_test,
                                                                                                       balanced_metrics,
                                                                                                       rng
                                                                                                       )
        ...
        val_train_edges, val_train_edges_false, _, _, test_edges, test_edges_false = mask_test_edges(adj, 0, ratio_test,
                                                                                                     balanced_metrics,
                                                                                                     rng)
        ...
        rng.shuffle(all_false_edge_idx)
        for i in range(splits):
    ...
    print("# imbalance ratio (non-edges/edges): " + str(
        int((adj.shape[0] * (adj.shape[0] - 1) - adj.todense().sum() + np.diag(adj.todense()).sum()) / (
                adj.todense().sum() - np.diag(adj.todense()).sum()))))
